...
session_states = defaultdict(lambda: {
    ...
    "emotion_trend": [],
    "confidence_trend": [],
    "current_entertainment": None,
    "patient_name": "User",
    "history": [] # V4.3: Added in-memory history to replace SQLite
})
...
# ==============================================================================
def classify_input(text, original_text, history, session_id=None):
    ...
    # === NEGATION GUARD (V4.1) ===
    # If model predicted 'happy' but input has negation words, override to 'sad'
    # This catches "not feeling good", "im not happy", "not okay" etc.
    negation_words = [r"\bnot\b", r"\bdont\b", r"\bdon't\b", r"\bdo not\b", r"\bcant\b", r"\bcan't\b", 
                      r"\bcan not\b", r"\bnever\b", r"\bnahi\b", r"\bna\b", r"\bnhi\b"]
    has_negation = any(re.search(neg, combined_check) for neg in negation_words)
    
    if emotion == "happy" and has_negation:
        # Check if input has ANY critical keywords first (safety!)
    ...
    # DO NOT trigger if negation words are present before positive words
    negation_words = [r"\bnot\b", r"\bdont\b", r"\bdo not\b", r"\bcant\b", r"\bcan not\b", r"\bnever\b", r"\bno\b", r"\bnahi\b", r"\bna\b"]
    has_negation = any(re.search(neg, combined_check) for neg in negation_words)
    
    if emotion not in ["happy"] and any(kw in combined_check for kw in happy_keywords) and not has_negation:
    ...
    # ==========================================================================
    # 🔄 EMOTION MOMENTUM GUARD (V4.2)
    # ==========================================================================
    # If the user has been in a consistent emotional state for 2+ turns,
    # don't let an ambiguous message suddenly flip to a completely different
    # emotion. This prevents "tu pahla hai" → happy when user was sad.
    if session_id and session_id in session_states:
        emotion_trend = session_states[session_id].get("emotion_trend", [])
        confidence_trend = session_states[session_id].get("confidence_trend", [])
        
        if len(emotion_trend) >= 2:
            recent_emotions = emotion_trend[-3:]  # last 3 turns
            
            # Count how many recent turns had a "negative" emotion
            negative_emotions = ["sad", "critical"]
            positive_emotions = ["happy"]
            neutral_emotions = ["chitchat", "out_of_context"]
            
            neg_count = sum(1 for e in recent_emotions if e in negative_emotions)
            
            # RULE 1: If user was consistently sad/critical and model now says happy
            #         with moderate confidence, keep them in the sad state.
            #         (Truly happy messages will have high confidence + happy keywords)
            if neg_count >= 2 and emotion in positive_emotions and confidence < 0.85:
                # Check: are there any strong happy keywords that justify the switch?
                strong_happy_signals = [
                    "khush", "happy", "excited", "amazing", "great",
                    "awesome", "maza", "best", "achha", "wonderful",
                    "celebrate", "won", "jeet", "success", "top",
                    "first rank", "placed", "selected", "scholarship",
                    "love you", "thank", "better now", "fine now",
                    "achha feel", "good now", "theek hu", "theek hoon",
                ]
                has_strong_happy = any(kw in combined_check for kw in strong_happy_signals)
                
                if not has_strong_happy:
                    # Override: maintain the negative emotional context
                    return "sad", max(confidence, 0.65), probabilities
            
            # RULE 2: If user was consistently sad/critical and model says chitchat/OOC
            #         with low confidence, keep them in sad context.
            if neg_count >= 2 and emotion in neutral_emotions and confidence < 0.70:
                return "sad", 0.60, probabilities
            
            # RULE 3: If user was consistently happy and model says sad with low confidence,
            #         keep them happy (prevents casual remarks from triggering sadness)
            pos_count = sum(1 for e in recent_emotions if e in positive_emotions)
            if pos_count >= 2 and emotion == "sad" and confidence < 0.70:
                has_strong_sad = any(kw in combined_check for kw in sad_keywords)
                if not has_strong_sad:
                    return "happy", max(confidence, 0.65), probabilities
...
def get_entertainment(category, session_id):
    ...
    if items:
        ...
        session_states[session_id]["current_entertainment"] = category
...
# ==============================================================================
def llm_fallback(user_input, risk_level, history, session_id=None):
    conversation_history = history
    """Only called when no other handler can respond. Uses Groq Llama 3.3.
    V4.2: Now injects emotional summary for deeply contextual responses."""
    ...
    # V4.2: Build emotional summary from session state
    emotional_summary = ""
    if session_id and session_id in session_states:
        emotion_trend = session_states[session_id].get("emotion_trend", [])
        ctx = session_states[session_id].get("current_context", "general")
        if emotion_trend:
            recent = emotion_trend[-3:]
            emotional_summary = f"\nEmotional trend (last {len(recent)} turns): {' → '.join(recent)}"
            if ctx != "general":
                emotional_summary += f"\nSpecific topic they're dealing with: {ctx}"
            # Detect if mood is sustained
            if len(recent) >= 2 and len(set(recent)) == 1:
                emotional_summary += f"\nIMPORTANT: User has been consistently {recent[0]} — respond with deeper empathy, don't give surface-level replies."
    
    system_prompt = f"""You are 'MindMate', a supportive, empathetic student companion chatbot.
RULES:
1. Keep responses SHORT (max 2-3 sentences).
2. Use 'Hinglish' (Hindi + English mix with emojis).
3. Be deeply empathetic — reference SPECIFIC things the user mentioned (their exact words, situations, feelings).
4. NEVER give generic advice like "sab theek ho jayega". Instead, acknowledge their SPECIFIC pain point.
5. NEVER suggest harmful actions.
6. If the user seems in distress, validate their feelings with SPECIFIC references to what they said.
7. Sound like a caring college friend who truly LISTENS, not a therapist reading from a script.
{emotional_summary}"""
...
def enhance_response(static_response, emotion, context, user_input, history, session_id=None):
    conversation_history = history
    """
    V4.3: FULL CONTEXTUAL RESPONSE GENERATION.
    Instead of "rephrasing" a generic template, Groq now generates the
    response FROM SCRATCH using the full conversation context. The static
    response is only used as a tone/intention guide.
    Falls back to static response if Groq API is unavailable.
    """
    global _recent_responses
    
    if API_ACTIVE and groq_client:
        try:
            # Build full conversation transcript (not just snippets)
            conversation_transcript = ""
            for h in conversation_history[-5:]:
                conversation_transcript += f"Student: {h['user']}\nMindMate: {h['bot']}\n"
            
            # V4.3: Build rich emotional context
            emotional_context = ""
            mood_instruction = ""
            if session_id and session_id in session_states:
                emotion_trend = session_states[session_id].get("emotion_trend", [])
                sub_ctx = session_states[session_id].get("current_context", "general")
                
                if emotion_trend:
                    recent = emotion_trend[-4:]
                    emotional_context += f"Emotional journey so far: {' → '.join(recent)}"
                    
                    # Sustained mood = go deeper
                    neg_count = sum(1 for e in recent if e in ["sad", "critical"])
                    if neg_count >= 2:
                        mood_instruction = """⚠️ This student has been struggling for multiple turns.
DO NOT give surface-level comfort. Show you've been LISTENING to their whole story.
Reference something from EARLIER in the conversation, not just their last message.
Make them feel truly HEARD and UNDERSTOOD."""
                    
                    # Mood shift
                    if len(recent) >= 2 and recent[-1] != recent[-2]:
                        mood_instruction += f"\n📍 Their mood just shifted from {recent[-2]} → {recent[-1]}. Acknowledge this shift naturally."
                
                if sub_ctx != "general":
                    emotional_context += f"\nThey're specifically dealing with: {sub_ctx}"
            
            # Build the list of things NOT to say
            avoid_list = ""
            if _recent_responses:
                recent_snippets = [r[:60] for r in _recent_responses[-6:]]
                avoid_list = f"\nDO NOT use similar wording to these recent responses:\n" + "\n".join(f"- {s}" for s in recent_snippets)
            
            generation_prompt = f"""You are MindMate — a deeply empathetic Hinglish mental health companion for Indian college students. You are NOT a therapist, you are their closest friend who genuinely CARES.
...
EMOTIONAL STATE:
{emotional_context}
Current classification: {emotion} | Topic: {context}

INTENTION (what your response should accomplish — use this as a GUIDE, not as text to copy):
"{static_response}"

{mood_instruction}

CRITICAL RULES:
1. Generate a COMPLETELY NEW response (2-3 sentences max). Do NOT copy or rephrase the intention text.
2. Use warm Hinglish (Hindi + English mix) naturally — like a real college friend texting.
3. You MUST reference something SPECIFIC the student said — their exact words, their situation, their feelings. 
   ❌ BAD: "Sab theek ho jayega" / "Main samajh sakta hu" / "Tu akela nahi hai" (TOO GENERIC)
   ✅ GOOD: "Exam fail hone ka dard real hai yaar, aur jab ghar pe bhi pressure ho toh..." (SPECIFIC)
4. Show EMOTIONAL INTELLIGENCE — match their energy. If they're exhausted, be gentle. If they're angry, validate the anger.
5. Include 1-2 relevant emojis, not more.
6. NEVER give unsolicited advice. First VALIDATE, then ONLY ask a gentle follow-up question if appropriate.
7. If they said something vague, gently ask what's specifically bothering them.
{avoid_list}

Your response (Hinglish, 2-3 sentences, deeply personal):"""
            
            response = groq_client.chat.completions.create(
                model=GROQ_MODEL,
                messages=[
                    {"role": "system", "content": "You are MindMate. You respond ONLY with the message itself. No labels, no quotes, no explanations."},
            ...
            # Clean up any wrapper formatting
            generated = generated.strip('"').strip("'").strip()
            # Remove any meta-labels like "MindMate:" or "Response:"
            for prefix in ["MindMate:", "Response:", "Bot:", "Reply:"]:
                if generated.lower().startswith(prefix.lower()):
                    generated = generated[len(prefix):].strip()
            
            if generated and 15 < len(generated) < 500:
                static_response = generated
        except:
...
def get_response(emotion, confidence, risk, context, cleaned_text, original_input, session_id, history):
    ...
    if risk == "CRITICAL":
        if context == "abuse" or session_states[session_id].get("current_context") == "abuse":
            if session_states[session_id]["step"] == "safety_check":
                if any(w in cleaned_text for w in ["yes", "haan", "han", "bilkul"]):
                    session_states[session_id]["step"] = None
            ...
            session_states[session_id]["step"] = "safety_check"
            session_states[session_id]["current_context"] = "abuse"
    ...
    if risk == "HIGH":
        ...
        if not safety_already_shown:
            ...
            validation = enhance_response(validation, emotion, context, original_input, history, session_id=session_id)
    ...
    if context in ["music", "joke", "movie"]:
        return get_entertainment(context, session_id)
    
    # Check if user wants more entertainment (broader matching)
    if session_states[session_id]["current_entertainment"]:
        if any(w in cleaned_text for w in ["yes", "haan", "han", "bilkul", "aur", "ek aur", "more"]):
            return get_entertainment(session_states[session_id]["current_entertainment"], session_id)
        if any(w in cleaned_text for w in ["no", "nahi", "nope", "bas", "mat", "enough"]):
            session_states[session_id]["current_entertainment"] = None
    ...
    if context == "help":
        ...
        last_context = session_states[session_id].get("current_context", "general")
    ...
    if emotion == "sad":
        ...
        session_states[session_id]["current_context"] = context if context != "general" else session_states[session_id].get("current_context", "general")
        validation = random.choice(RESPONSES["sad"]["validation"])
        validation = enhance_response(validation, emotion, context, original_input, history, session_id=session_id)
        followup = random.choice(RESPONSES["sad"]["followup"])
        return validation + "\n\n" + followup
    ...
    if emotion == "happy":
        ...
        return enhance_response(happy_resp, emotion, context, original_input, history, session_id=session_id)
    
    # ========== FALLBACK: Gemini or Generic ==========
    llm_response = llm_fallback(original_input, risk, history, session_id=session_id)
    if llm_response:
        return llm_response
...
def generate_reply(session_id, user_message):
    # Fetch recent history from in-memory session (last 3 turns)
    history = session_states[session_id]["history"][-3:]
    
    cleaned = clean_text(user_message)
    emotion, confidence, probs = classify_input(cleaned, user_message, history, session_id=session_id)
    ...
    # V4.2: Track emotion and confidence trends for momentum
    session_states[session_id]["emotion_trend"].append(emotion)
    if len(session_states[session_id]["emotion_trend"]) > 5:
        session_states[session_id]["emotion_trend"] = session_states[session_id]["emotion_trend"][-5:]
    
    session_states[session_id]["confidence_trend"].append(confidence)
    if len(session_states[session_id]["confidence_trend"]) > 5:
        session_states[session_id]["confidence_trend"] = session_states[session_id]["confidence_trend"][-5:]
    ...
    # Save the turn to in-memory history
    ...
    if len(session_states[session_id]["history"]) > 20:
        session_states[session_id]["history"].pop(0)
...
def main():
    ...
    session_states[session_id]["patient_name"] = patient_name
    ...
    while True:
        try:
            ...
            # Use the refactored generate_reply to process everything
            bot_reply, emotion, risk, confidence = generate_reply(session_id, user_input)
